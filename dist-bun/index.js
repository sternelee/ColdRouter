...
var LOG_DIR2 = join4(homedir3(), ".openclaw", "clawrouter", "logs");
var dirReady = false;
...
async function logUsage(entry) {
  try {
    ...
    const file = join4(LOG_DIR2, `usage-${date}.jsonl`);
...
function injectModelsConfig(logger) {
  ...
  const configPath = join5(configDir, "openclawon");
  let config = {};
...
async function startProxyInBackground(api, apiKeys) {
...
async function createKeysCommand(apiKeys) {
  return {
    name: "keys",
...
var plugin = {
  ...
  register(api) {
    ...
    startProxyInBackground(api, apiKeys).then(async () => {
